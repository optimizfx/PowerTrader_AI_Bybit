    ...
    try:
        ...
        for cat in ["spot", "linear", "inverse", "option"]:
            try:
                ...
                if cat == "spot":
                    # Spot uses account balance instead of positions
                    # This might still error depending on account type, but we'll catch it
                    try:
                        bal = session.get_wallet_balance(accountType="UNIFIED")
                        print(f"Spot Balance: Check 'result.list[0].coin' for assets")
                    except:
                        print("Spot Balance: Account balance check not available or requires different permissions.")
                else:
                    params = {"category": cat, "limit": 10}
                    if cat in ["linear", "inverse"]:
                        # Bybit V5 requires symbol or settleCoin for these categories
                        params["settleCoin"] = "USDT"
                        
                    pos = session.get_positions(**params)
                ...
                print(f"Error checking {cat}: {cat_e}")
